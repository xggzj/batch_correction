# Read infiles
library(flowCore)
file_1 = read.FCS("000027_d_C4.fcs")
file_2 = read.FCS("VG32_(tv.1)-PTc_d_C1.fcs")

# A function to read the short names from a flowFrame
...
# Concatenate two batches with cells in the rows and features in the columns
dat = rbind(batch_1, batch_2)
# Batch covariate
batch = as.factor(c(rep(1, nrow(batch_1)), rep(2, nrow(batch_2))))
...
KBET = function(pc20, Batch){
  # k input value equal to 5%, 10%, 15%, 20%, and 25% of the sample size
...
ASW20 = function(pc20){
  ...
  pamk.best = pamk(pc20)
  asw20 = c()
  # Repeat ASW for 20 times and calculate the median of 20 ASW scores
  for (i in 1:20){
